...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--target", type=str, default="rk3566", help="RKNPU target platform"
    )
    parser.add_argument(
        "--npu_device_test",
        action="store_true",
        default=False,
        help="Connected npu device run",
    )
    parser.add_argument(
        "--accuracy_analysis",
        action="store_true",
        default=False,
        help="Accuracy analysis",
    )
    parser.add_argument(
        "--eval_perf",
        action="store_true",
        default=False,
        help="Time consuming evaluation",
    )
    parser.add_argument(
        "--eval_memory", action="store_true", default=False, help="Memory evaluation"
    )
    parser.add_argument("--model", type=str, default=MODEL_PATH, help="onnx model path")
    parser.add_argument(
        "--output_path", type=str, default=OUT_RKNN_PATH, help="output rknn model path"
    )
    parser.add_argument(
        "--dtype",
        type=str,
    ...
    if args.target in RKNPU1_TARGET:
        ret = rknn.load_onnx(
            model=args.model, inputs=["input"], input_size_list=[[3, 224, 224]]
        )
    else:
        ret = rknn.load_onnx(
            model=args.model, inputs=["input"], input_size_list=[[1, 3, 224, 224]]
        )
