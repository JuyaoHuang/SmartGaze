...
def letterbox_resize(image, size, bg_color):
    ...
    result_image[offset_y : offset_y + new_height, offset_x : offset_x + new_width] = (
        image
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="RetinaFace Python Demo", add_help=True
    )
    # basic params
    parser.add_argument(
        "--model_path", type=str, required=True, help="model path, could be .rknn file"
    )
    parser.add_argument(
    ...
    boxes[..., 0::2] = np.clip(
        (boxes[..., 0::2] - offset_x) / aspect_ratio, 0, img_width
    )  # letterbox
    boxes[..., 1::2] = np.clip(
        (boxes[..., 1::2] - offset_y) / aspect_ratio, 0, img_height
    )  # letterbox
    scores = conf.squeeze(0)[:, 1]  # face score
    landmarks = decode_landm(landmarks.squeeze(0), priors)  # face keypoint data
    scale_landmarks = np.array(
        [
            model_width,
            model_height,
            model_width,
    ...
    landmarks[..., 0::2] = np.clip(
        (landmarks[..., 0::2] - offset_x) / aspect_ratio, 0, img_width
    )  # letterbox
    landmarks[..., 1::2] = np.clip(
        (landmarks[..., 1::2] - offset_y) / aspect_ratio, 0, img_height
    )  # letterbox
    # ignore low scores
